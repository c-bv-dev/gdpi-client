    ...
    return (
        <nav className='bg-gray-900 p-1 w-64 flex flex-col gap-2 relative h-full'>
            ...
            <div className='flex pt-6 flex-col gap-3'>
                {
                    links.map((link) => (
                        <Link to={link.link} className='flex p-1 px-1 rounded items-center gap-2'>{link.icon}{link.name}</Link>
                    ))
                }
            </div>
            <div className='flex gap-4 px-3 absolute bottom-3 items-center'>
                <img src="https://images.assetsdelivery.com/compings_v2/nexusplexus/nexusplexus2101/nexusplexus210100387.jpg" alt="" className='w-10 h-10 rounded-full' />
